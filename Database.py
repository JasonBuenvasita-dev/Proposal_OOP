...
index b27f8e3c62f735d0581d55657d61eb6034ca3950..5db2e055d170cbaa263a4b68ce9976787b803732 100644
--- a/Database.py
+++ b/Database.py
@@ -1,123 +1,207 @@
...
+    def load_tasks(self):
+        try:
+            self.status_var.set("Loading tasks from database...")
+            self.root.update_idletasks()
+            response = self.http.get(self.api_url, timeout=8)
+            response.raise_for_status()
+            result = response.json()
+
+            if result.get("status") != "success":
+                self.status_var.set("Could not load tasks.")
+                return
+
...
+                    ),
+                )
+            self.status_var.set("Connected. Tasks loaded.")
+        except requests.exceptions.RequestException:
+            self.status_var.set("Database connection unavailable.")
+
+    def add_task(self):
